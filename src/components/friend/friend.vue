<template>
    <div class="f_container">
        <div class="f_nav">
            <div class="f_nav_n1">
                <svg t="1584330809456" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="8538" width="30" height="30"><path d="M833.829799 308.87372c0 0.340761 0.475837 2.049682 0.475837 2.390443 0.249687 3.095501 0.249687 3.095501 0.412392 5.282307 0.895393 12.001335-3.498684 23.539112-7.835456 34.441417-11.423167 28.713972-32.389688 49.926086-56.419987 68.225871-10.372232 7.898901-22.550599 12.869099-35.141358 16.527418-8.147564 2.367931-16.154935 5.445012-24.449855 7.034207-15.255449 2.922563-30.664394 5.057179-46.031383 7.375991-11.911284 1.797949-23.859407 3.349281-35.782971 5.061272-10.594289 1.521656-21.342074 2.411933-31.725562 4.848425-13.56597 3.183506-26.862811 7.535627-40.219004 11.571546-3.079128 0.930185-4.468778 0.924045-4.832052-2.902096-0.677429-7.142677-2.054799-14.21577-2.953262-21.342074-2.095731-16.627702-4.436032-33.236985-5.996574-49.916876-1.243317-13.283538-1.992377-26.659173-2.048659-39.997969-0.092098-21.584598 0.080841-23.700795 0.988514-64.754816 0.418532-18.913768 2.240017-37.784558 2.895957-56.692187 0.49528-14.298658 3.573385-27.744901 10.525728-40.266076 8.354272-15.046694 20.255323-26.919093 35.230386-34.870182 22.354124-11.868305 46.248324-15.69547 71.052243-8.524141 17.71957 5.123694 30.18037 16.555048 37.821397 33.31578 4.984524 10.934027 6.735401 22.397103 6.217608 34.350342-0.112564 2.597151 0.989537 5.246491 1.612731 8.264221 17.006326-8.510838 33.838689-12.780071 51.957349-11.594059 9.976212 0.652869 19.076474 4.222161 27.220968 9.18929 12.259208 7.476275 21.799492 18.055215 29.037337 30.653137 6.180769 10.758018 11.106965 21.973454 13.96199 34.054607C831.237765 296.66977 832.493362 302.780955 833.829799 308.87372z" p-id="8539" fill="#72767d"></path><path d="M566.627155 650.006144c-5.616928-26.831089-34.767851-136.169309-38.551014-162.870438 0 0-0.001023 0.002047-0.002047 0.00307 0-0.001023 0.001023-0.002047 0.001023-0.002047-32.091906-3.352351-63.609737 1.434675-94.690616 8.266267-51.702546 8.700149-144.868435 62.120826-144.868435 62.120826-36.282345 23.311939-44.700061 69.040423-45.902446 85.987397s1.820462 65.87943 17.170055 86.687338c21.029966 28.507264 60.682058 40.11872 92.530416 29.238928 0.001023 6.625908 2.311649 12.703323 4.77884 18.73162 1.866511 4.016477 3.208065 8.246825 5.009084 12.295024 23.651676 53.173037 70.78209 74.621535 126.193097 57.124022 37.31793-11.784394 63.010079-36.272112 77.623915-72.466452 3.377934-12.236696 6.138814-24.561395 5.331425-37.395702C575.971987 708.165752 572.656475 678.80505 566.627155 650.006144z" p-id="8540" fill="#72767d"></path><path d="M268.164552 488.245993c23.442922-0.572028 46.937009 0.694825 70.098521-3.959172 59.803038-12.016685 119.843482-21.030989 181.136453-15.331173l0-0.001023 0.001023 0.001023c14.363126 1.540076 11.919471-3.246951 11.272741-8.918114-2.362814-20.727067-7.65126-41.306778-10.600429-61.956074-7.504928-52.552913-14.681374-105.202017-34.039257-155.162896-12.593829-32.503275-35.631522-53.06559-69.839625-60.526515-85.505419-18.647709-151.844314 39.426965-143.782707 125.758192-36.83186-0.032746-69.628824 8.071839-87.160105 45.036729C161.248498 403.799696 202.356754 489.852584 268.164552 488.245993z" p-id="8541" fill="#72767d"></path><path d="M859.124905 513.953492c-11.625782-20.644179-34.314527-31.375591-56.104809-37.894052-42.528606-12.720719-85.819574-14.956643-129.799228-9.895371-35.818787 4.122901-64.11525 8.571213-122.514311 17.531282-7.91118 0.823761-5.192256 5.462408-4.050246 9.743922 5.506411 20.654412 12.515035 52.173267 15.589047 62.210877 10.981099 33.9226 33.835619 100.003622 49.881061 147.57201 2.468215 5.040806 22.843264 44.828998 37.634132 53.703109 30.982642 18.587334 63.309908 15.289218 92.991927-0.825808 31.398104-17.046235 46.078455-45.461401 40.902572-82.233909-2.298346-6.55223-4.596691-13.103436-6.89606-19.654642 0.174985 0.107447 0.343831 0.223081 0.518816 0.331551 5.375427 1.89107 10.92277 3.89982 16.535605 3.988848 22.997784 0.367367 45.299719-1.611707 59.855226-23.225981 10.943236-14.651698 18.032702-30.096459 21.818935-48.892547C880.525308 561.412386 868.88213 531.139919 859.124905 513.953492z" p-id="8542" fill="#72767d"></path></svg>
            </div>
            <h3>好友</h3>
            <div class="f_nav_slash"></div>
            <div class="f_nav_select" @click="nav_click('friend_online')" v-bind:class="{f_nav_select_highlight:var_nav_click=='friend_online'}">在线</div>
            <div class="f_nav_select" @click="nav_click('friend_all')" v-bind:class="{f_nav_select_highlight:var_nav_click=='friend_all'}">全部</div>
            <div class="f_nav_select" @click="nav_click('friend_waiting')" v-bind:class="{f_nav_select_highlight:var_nav_click=='friend_waiting'}">待定</div>
            <div class="f_nav_select" @click="nav_click('friend_shield')" v-bind:class="{f_nav_select_highlight:var_nav_click=='friend_shield'}">已屏蔽</div>
            <div class=" f_nav_select_addfriend" @click="nav_click('add_friend')" v-bind:class="{f_nav_select_add_highlight:var_nav_click=='add_friend'}">添加好友</div>
            <div></div>
            <div></div>
        </div>
        <div class="f_plain">
            <div class="f_plain_right">
                <component v-bind:is="var_nav_click"></component>
            </div>
            <div class="f_plain_left">
                <component v-bind:is="'now_active'"></component>
            </div>
        </div>
        
    </div>
</template>
    
<script>
import friend_online from './friend_online'
import friend_all from './friend_all'
import friend_waiting from './friend_waiting'
import add_friend from './add_friend'
import friend_shield from './friend_shield'
import now_active from './now_active'
export default {
    name: 'friend',
    data: function(){
        return{
            var_nav_click: 'friend_online'
        }
    },
    methods:{
        nav_click: function(e){
            this.var_nav_click = e
        }
    },
    components:{
        friend_online,
        friend_all,
        friend_waiting,
        add_friend,
        friend_shield,
        now_active
    }
}
</script>

<style>
:root{
    --elevation-low:0 1px 0 rgba(4,4,5,0.2),0 1.5px 0 rgba(6,6,7,0.05),0 2px 0 rgba(4,4,5,0.05)
}

.f_container{
    height: 100%;
    width: 100%;
    display: grid;
    grid-template-rows: 48px auto;
    color: white;
}
.f_nav{
    width: 100%;
    height: 48px;
    box-shadow: var(--elevation-low);
    display: flex;
    flex-direction: row;
    /* justify-content: center; */
    align-items: center;
    background-color: #36393f;
    z-index: 1;
}

.f_nav_slash{
    width: 1px;
    height: 24px;
    background-color: rgba(83, 83, 83, 0.5);
    margin: 0 8px 0 16px;
    flex: 0 0 auto;
}
.f_nav_n1{
    margin: 0px 4px;
    margin-left: 12px;
}
.f_nav h3{
    display: block;
    font-size: 1em;
    margin-block-start: 1em;
    margin-block-end: 1em;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
    font-weight: bold;
    white-space: nowrap;
    text-overflow: ellipsis;
    overflow: hidden;
    -ms-flex-negative: 0;
    flex-shrink: 0;
    /* font-weight: 500; */

}

.f_nav_select{
    margin: 0px 8px;
    color: rgb(173, 173, 173);
    padding: 1px 8px 1px 8px;
    border-radius: 5px;
    white-space: nowrap;
    text-overflow: ellipsis;
    overflow: hidden;
    -ms-flex-negative: 0;
    flex-shrink: 0;
    font-weight: 500;

}

.f_nav_select:hover{
    background-color: rgba(204, 204, 204, 0.06);
    /* color: white; */
}

.f_plain{
    height:100%;
    width: 100%;
    display: grid;
    grid-template-columns: auto 360px;
}
.f_plain_right{
    height: 100%;
}
.f_plain_left{
    height: 100%;
    background-color: #2f3136;
}

@media screen and (max-width:1200px){ 
    .f_plain {grid-template-columns: auto;}
    .f_plain_left{ display: none;}
}
.f_nav_select_highlight{
    color: white;
    background-color: rgba(204, 204, 204, 0.06);
}
.f_nav_select_addfriend{
    color: white;
    background-color:#43b581;
    margin: 0px 8px;
    padding: 1px 8px 1px 8px;
    border-radius: 5px;
    white-space: nowrap;
    text-overflow: ellipsis;
    overflow: hidden;
    -ms-flex-negative: 0;
    flex-shrink: 0;
    font-weight: 500;

}

.f_nav_select_add_highlight{
    color: #43b581;
    background-color: #38524c;
}
</style>