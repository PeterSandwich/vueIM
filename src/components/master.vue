            ...
            <div class="sideBar_tool">
                <div class="sideBar_list">
                    ...
                    <div class="sideBar_select sideBar_select_h" @mouseover="sideBar_select_hover='info'" @click="sideBar_select_click('info')" @mouseleave="sideBar_select_hover=0" v-bind:class="{sideBar_select_hover_active: sideBar_select_hover=='info'&&sideBar_select_select!='info', sideBar_select_select_active:sideBar_select_select=='info'}">
                        <div>
                            <svg t="1584713507863" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4517" width="28" height="28"><path d="M513 519m-184 0a184 184 0 1 0 368 0 184 184 0 1 0-368 0Z" p-id="4518" v-bind:fill="icon_color('info')"></path><path d="M856.7 373.8c-18.8-44.4-45.7-84.3-79.9-118.6l-31.1 31.1C807.8 348.5 842 431.1 842 519c0 181.4-147.6 329-329 329S184 700.4 184 519c0-160.7 115.9-294.9 268.5-323.4 10.8 24.1 34.9 40.9 63 40.9 38.1 0 69-30.9 69-69s-30.9-69-69-69c-32.6 0-59.9 22.6-67.1 53.1-27.6 4.8-54.5 12.8-80.6 23.8-44.4 18.8-84.3 45.7-118.6 79.9s-61.1 74.1-79.9 118.6c-19.5 46-29.3 94.9-29.3 145.2 0 50.3 9.9 99.2 29.3 145.2 18.8 44.4 45.7 84.3 79.9 118.6 34.2 34.2 74.1 61.1 118.6 79.9 46 19.5 94.9 29.3 145.2 29.3 50.3 0 99.2-9.9 145.2-29.3 44.4-18.8 84.3-45.7 118.6-79.9 34.2-34.2 61.1-74.1 79.9-118.6 19.5-46 29.3-94.9 29.3-145.2s-9.9-99.3-29.3-145.3z" p-id="4519" v-bind:fill="icon_color('info')"></path></svg>
                            ...
                            >
                                <div class="maspop_can">
                                     <div class="maspop_can_head">
                                         ...
                                         <div class="maspop_can_head_input" ref="maspop">
                                             <div class="maspop_can_head_input_item" v-for="i in 5" :key="i.id">
                                                <div>cheng long{{i}}</div>
                                                <span><svg t="1585204143380" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6225" width="12" height="12"><path d="M512 620.544l253.3376 253.3376a76.6976 76.6976 0 1 0 108.544-108.544L620.6464 512l253.2352-253.3376a76.6976 76.6976 0 1 0-108.544-108.544L512 403.3536 258.6624 150.1184a76.6976 76.6976 0 1 0-108.544 108.544L403.3536 512 150.1184 765.3376a76.6976 76.6976 0 1 0 108.544 108.544L512 620.6464z" p-id="6226" fill="#c09282"></path></svg></span>
                                            </div>
                                            <input placeholder="查找好友"/>
                                         </div>
                                     </div>
                                     <div class="maspop_can_body">
                                        <div v-for="i in 23" :key="i.id" class="maspop_can_body_item">
                                            <user-head style="height: 32px;width: 32px;" v-bind:with_status="true"  v-bind:user_status="i%4+1" v-bind:bg_color="'#2f3136'"></user-head>
                                            <div class="maspop_can_body_item_name">Arvin_Lin<span>#1866</span></div>
                                            <div>
                                ...
                                <div slot="reference" style="color: rgb(168, 168, 168);">
                                    <el-tooltip class="item" effect="dark" content="创建私信" placement="top"  :enterable="false">
                                        <svg x="0" y="0" class="privateChannelRecipientsInviteButtonIcon-cXLJ7f icon-22AiRD" aria-hidden="false" width="16" height="16" viewBox="0 0 18 18"><polygon fill-rule="nonzero" fill="currentColor" points="15 10 10 10 10 15 8 15 8 10 3 10 3 8 8 8 8 3 10 3 10 8 15 8"></polygon></svg>
                                    </el-tooltip>
                                </div>
                        </el-popover>          
                    </div>
                    <div class="sideBar_select" v-for="chat_item in getChatList" v-bind:key="chat_item.chat_id" @click="sideBar_select_click('chat',chat_item.chat_id)" @mouseover="sideBar_select_hover=chat_item.chat_id"  @mouseleave="sideBar_select_hover=0" v-bind:class="{sideBar_select_hover_active: sideBar_select_hover==chat_item.chat_id&&sideBar_select_select!=chat_item.chat_id, sideBar_select_select_active:sideBar_select_select==chat_item.chat_id}">
                        <div class="sideBar_select_w">
                            <user-head style="height: 32px;width: 32px;" v-bind:with_status="!chat_item.is_group"  v-bind:user_status="chat_item.user_status" v-bind:bg_color="'#2f3136'"></user-head>
                ...
                <div class="sideBar_me_in">
                    <div style="position:relative;"  ref="changing_status_el">
                        <div class="sideBar_me_head" @click="changing_status = !changing_status"  @mouseover="head_hover=true" @mouseleave="head_hover=false" >
                            ...
                            <user-head style="height: 32px;width: 32px;" v-bind:with_status="true"  v-bind:user_status="me_status" v-bind:bg_color="'#2f3136'"></user-head>
                        </div>
                    ...
                    <div class="msdb_2">
                        <div class="master_setting_dialog_close">
                            <div @click="dialogSettingVisible=false">
                                <svg t="1585215681448" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="13482" width="34" height="34"><path id="svg_1" fill="#e6e6e6" d="m7.67,512.003c0,282.77 227.196,511.997 507.456,511.997c280.27,0 507.457,-229.227 507.457,-511.997s-227.187,-512.003 -507.456,-512.003c-280.26,0 -507.458,229.233 -507.458,512.003l0.001,0z"/>
  <path stroke="null" id="svg_2" fill="#d81e06" d="m729.1236,298.11762c11.5673,11.41216 11.5673,31.37174 0,42.78849l-390.8125,384.97836c-11.58724,11.38915 -31.85129,11.38915 -43.43469,0c-11.5673,-11.40832 -11.5673,-31.37634 0,-42.78849l390.80943,-384.97759c11.58647,-11.39299 31.85512,-11.39299 43.43776,0l0,-0.00077z"/>
  <path stroke="null" id="svg_3" fill="#d81e06" d="m301.40334,299.80205c11.23808,-11.30459 30.8915,-11.30459 42.12958,0l379.06583,381.94742c11.21432,11.3242 11.21432,31.13253 0,42.45203c-11.23885,11.29988 -30.8915,11.29988 -42.13418,0l-379.06046,-381.94742c-11.21815,-11.32891 -11.21815,-31.13253 0,-42.45203l-0.00077,0z"/></svg>
                            </div>
                            <h5>
                                ESC
                            </h5>
...
export default {
    ...
    methods:{
        ...
        sideBar_select_click:function(e,i){
            if(e=='chat'){

                // this.sideBar_select_select=i
                this.$router.push({name: 'chat', params:{id: i}}).catch(()=>{})
                console.log(e)
            }else{
                ...
                if(e=='friend'){
                    ...
                    this.$router.push({name: 'infomation'}).catch(()=>{})
                }
            }      
